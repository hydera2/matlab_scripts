    ...
    if nargin < 1 % No file was identified in the call
        ...
        if (boolfiles == 1)
            ...
            if strcmpi(r.CurryLocations, 'True')
                ...
                EEG.history = sprintf('%s\nEEG = loadcurry(''%s%s'', ''CurryLocations'', ''False'');', EEG.history, filepath, [name, datafileextension]);
            end
                ...
                if ~isempty(impedancematrix)
                    if (size(impedancematrix,2) == size({EEG.chanlocs.labels},2)) % number of channels matches number of impedances
                        ...
                        for cC = 1:size({EEG.chanlocs.labels},2)
                           EEG.chanlocs(cC).impedance = impedancematrix(1,cC); 
                           EEG.chanlocs(cC).median_impedance = impedancelist(1,cC);
                        end
                    end
                end
            catch
            ...
            if ~isempty(events)
                ...
                for cC = 1:size(events,2)
                    ...
                    EEG.event(cC).latency = events(1,cC);
            ...
            if (nTrials > 1)
                ...
                if ~(exist([file '.psydat'], 'file') == 0) 

                    fid = fopen([file '.psydat'],'rt');
                    if (fid ~= -1)
                        cell = textscan(fid,'%s');
                        fclose(fid);
                        cont = cell{1};
